...
To create a state, pass a **configuration** object to the [`useStateDesign`](/docs/api/useStateDesign) hook.
...
> **Tip:** If you want to use the same state with more than one component, you can create a state with the [`createStateDesign`](/docs/api/useStateDesign) function, then pass the state to the `useStateDesign` hook. To learn more, check out the [`useStateDesign`](/docs/api/useStateDesign) article.
