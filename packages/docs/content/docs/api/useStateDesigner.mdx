...
If you pass `useStateDesigner` a **state** that you've created somewhere else (such as with [`createStateDesigner`](/docs/api/createStateDesigner)) then the hook will subscribe the component to the state's updates.

```jsx
import { createStateDesigner } from "@state-designer/core"
import { useStateDesigner } from "@state-designer/react"

const counter = createStateDesigner({ data: { count: 1 } })
