...
```ts
useStateDesign(source: state | config, dependencies?: any[]): state
```

The `useStateDesign` hook will subscribe a component to a state's updates. Any time the state updates, so will the component.
...
If you pass `useStateDesign` a **state** that you've created somewhere else (such as with [`createStateDesign`](/docs/api/createStateDesign)) then the hook will subscribe the component to the state's updates.

```jsx
import { createStateDesign } from "@state-designer/core"
import { useStateDesign } from "@state-designer/react"

const counter = createStateDesign({ data: { count: 1 } })
...
You can also pass `useStateDesign` a **configuration object** instead. In this case, the hook will use the config to create a _new_ state and then subscribe to its updates.
...
function Example() {
  // highlight-next-line
  const state = useStateDesign({ data: { count: 1 } })
  return <h1>{state.data.count}</h1>
}
```
...
If you're using the internal state pattern, you can pass the `useStateDesign` hook a second argument: an array of dependencies. If the hook finds changes in these dependencies between renders, it will rebuild a new state based on the current config.
...
function Parent() {
  const { data } = useStateDesign(counter)
  return <Child />
}

function Child(props) {
  const { send } = useStateDesign(counter)
  return <button onClick={() => send("SOME_EVENT")} />
}
```

You can subscribe a component to its parent's internal state, too. When using the internal state pattern, pass down the state returned by the `useStateDesign` hook. Other components can then subscribe to this state using the external state pattern.

```jsx
function Parent() {
  const localState = useStateDesign({ ... })
  return <Child state={localState}/>
}

function Child({ state }) {
  const { send } = useStateDesign(state)
  return <button onClick={() => send("SOME_EVENT")} />
}
```
...
The state returned from `useStateDesign` is a mix of immutable and mutable data. Each update returned from the hook is a new object comprised of the source state's immutable `data`, `active`, and `stateTree` properties, together with the source state's original stable methods, such as `send`, `isIn`, and `onUpdate`.
