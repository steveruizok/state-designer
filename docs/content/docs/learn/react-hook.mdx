...
In the last article, we learned about responding to a state's updates through the state's `onUpdate` method. That's fine for a regular JavaScript project—and we could use this method in a React project, too. However, to make things easier, State Designer's [React package](/docs/usage/react) includes a hook named <b>`useStateDesigner`</b> that does most of the work for us.

## Global and Local States

The `useStateDesigner` hook is designed to work in two different ways:

1.  If you pass it a **state** that you've created with `createStateDesigner`, the hook will subscribe the component to the state's updates.
